...
- **Disassemble** previews the return from a completed Workbench item in the Party Stash. Confirming permanently removes the item and returns **90% of each consumed material**, rounded down to whole resource units (10 Steel Ingots return 9; one unit returns zero). Matching material, tier, and dragon-colour entries are combined before rounding. Recorded Mark ingredients are included, but Marks, magical properties, and downtime are not recovered. Single-item projects only: unstack, unequip, and uninvest the item first. Unrecorded items and ambiguous duplicates are not eligible. An active GM processes the transaction; players need permission to edit the Party Stash. Returned resources are separate stacks, leaving existing reservations untouched. Disassembled projects stay in history and cannot use **Recover Missing Item**.
