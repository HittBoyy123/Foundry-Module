...
Download `wrathmaker-v0.19.1.zip` from the latest GitHub release and extract its contents into a folder named `pf2e-crafting-material-tiers` under Foundry's `Data/modules` folder. Restart Foundry and enable **Wrathmaker**.
...
Artisan slots show their required **professions** (for example, Blacksmithing or Leatherwork); optional slots show Any Profession. New items may have **one zero-Capacity Artisan Mark total**, regardless of how many artisans contribute. A GM Capacity override does not bypass this limit. Existing finished gear is not stripped of Marks. Mark choices and selected badges identify their profession and specialisation, or Universal for profession-wide Marks.

The **Wrathmaker Artisan Marks** reference compendium, inside **Pathfinder 2E Wrathmaker**, contains all 253 Marks. Search by Mark, profession, or specialisation name. Each journal includes the effect description, source, grade, Capacity, Core/Anchor requirements, and listed materials. These are reference entries, not feats to drag onto a character.

GMs can also use **Create Custom Item** in the Items Directory or **Wrathmaker GM Item Creator** in Module Settings. Drop a base item, name it, choose its Core and secondary materials/Tiers, and add compatible Artisan Marks with typed maker names (no Actors required). Each Mark keeps its rules text and supported automation. Capacity overflow requires an explicit GM confirmation; incompatible items, tiers, Anchors, and duplicate/stacking conflicts cannot be overridden. This creates a new world item without spending resources or downtime and works while the experimental Workbench is disabled. It does not create a crafting project, so project recovery does not apply. Disassembly can calculate a clearly labelled standard-recipe return for these items.

The **Crafting Workbench (Experimental)** switch in **Wrathmaker Game Settings** is off by default, including for existing worlds that have not opted in. A GM must enable it before players or GMs can use the Workbench. Turning it off hides the party/directory launch buttons and blocks new reservations, downtime advancement, completion, and recovery; saved projects and reservations remain intact for later use. Existing gear and its bonuses are unchanged. The separate **Crafting material rules** and **Resource gathering** switches retain their existing functions.

Open **Wrathmaker Workbench** from the Items Directory header or its Module Settings entry. Its tabs provide:
...
- **Disassemble** accepts dragged Wrathmaker-formatted items from character inventories or the Party Stash; GMs may also use world items. Completed items use their original project ledger. Unrecorded items, including GM-created gear, use a clearly labelled standard-recipe return based on their material, Tier, components, and Marks. Confirming permanently removes the original item and returns **90% of materials** to the selected Party Stash, rounded down to whole units (10 Steel Ingots return 9; one unit returns zero). Matching material, tier, and dragon-colour entries are combined before rounding. Marks, magical properties, and downtime are not recovered. Unstack, unequip, and uninvest the item first; only single-item recipes are supported. Compendium entries must first be imported, and ordinary items without Wrathmaker data are rejected. An active GM processes the transaction and rechecks the player's permission to modify both the source inventory and destination party. New resource stacks leave existing reservations untouched. Disassembly is recorded in project history and blocks **Recover Missing Item**. Tracked items whose original project is unavailable are blocked rather than falling back to an estimate.
...
For a completed project whose output has gone missing, the GM can choose **Recover Missing Item** to recreate it in the Party Stash without consuming resources or downtime. Recovery refuses if the output still exists in the world or an actor's inventory, including when it has been transferred to a character. Newly completed projects retain the original crafted output snapshot, including Marks and maker provenance; later edits to that item are not part of the snapshot. Older projects require the original base item to remain available and are rebuilt using their saved crafting data. Keep the completed project record if you want this recovery option.
