...
test("profession items provide three future specialties and a relevant +2 modifier", () => {
  for (const item of PROFESSION_ITEM_SOURCES) {
    ...
    assert.match(item.system.description.value, /trained at level 1, expert at level 4, master at level 10, and legendary at level 16/i);
  }
});

test("profession Lore advances at the requested character levels", () => {
  assert.deepEqual([1, 3, 4, 9, 10, 15, 16, 20].map(professionRankForLevel), [1, 1, 2, 2, 3, 3, 4, 4]);
  const lore = createProfessionLoreSource(getProfessionData(professionItem("blacksmithing")), 10);
  assert.equal(lore.type, "lore");
  assert.equal(lore.name, "Blacksmithing Lore");
    ...
    const actor = {
      ...
      async updateEmbeddedDocuments(_type, updates) {
        for (const update of updates) {
          ...
          if (item && Object.hasOwn(update, "system.proficient.value")) {
