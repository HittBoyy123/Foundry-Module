...
test("version 11 rules migrate with gathering enabled", () => {
  const legacy = cloneDefaultRulesConfig();
...
test("version 12 gathering rules migrate to region-aware party-stash rewards", () => {
  const legacy = cloneDefaultRulesConfig();
...
test("version 13 rules gain profession settings and canonical permanent flanking", () => {
  const legacy = cloneDefaultRulesConfig();
  legacy.schemaVersion = 13;
  delete legacy.professions;
...
test("legacy flanking configuration migrates to the permanent campaign rule", () => {
  const customized = cloneDefaultRulesConfig();
  customized.flanking.penalties[3] = -5;
  customized.flanking.penalties[4] = -6;
  customized.flanking.oversizedParticipantsPerSide = 3;
  customized.flanking.enabled = false;
  const normalized = normalizeRulesConfig(customized);
  assert.deepEqual(normalized.flanking.penalties, { 2: -2, 3: -3, 4: -4 });
  assert.equal(normalized.flanking.oversizedParticipantsPerSide, 2);
  assert.equal(normalized.flanking.enabled, true);
