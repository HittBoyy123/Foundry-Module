...
export const MARK_ACTIVATIONS = Object.freeze({
  ...
  ].map(id => [id, { type: "action", value: 1, note: "activate with one action. Follow the source's frequency and duration." }])),
  ...Object.fromEntries([
  ...
  ].map(id => [id, { type: "action", value: 2, note: "activate with two actions. Follow the source's frequency and duration." }])),
  "enchanting-specialty-3-grand-glyph": { type: "action", value: 3, note: "three actions to place the rune zone; choose its approved effect beforehand." },
  ...Object.fromEntries([
  ...
  ].map(id => [id, { type: "reaction", value: null, note: "spend your reaction when the trigger in the description occurs." }])),
  ...Object.fromEntries([
  ...
  ].map(id => [id, { type: "free", value: null, note: "free action at the described trigger or immediately before the affected check. Frequency limits still apply." }])),
